...
const BottomSheet = ({image, onBackToCamera}) => {
  console.log(image);
  ...
  return imageData?.success ? (
    <View style={styles.baseView}>
      ...
      <Image source={{uri: imageData?.image.uri}} style={styles.imageView} />
      <Text style={styles.name}>{capitalizeLetters(userInfo?.name)}</Text>
      <View style={styles.timeStamp}>
        <Icon name="watch" size={15} color="steelblue" />
        <Text style={styles.info}>{formatIsoDate(imageData?.timestamp)}</Text>
      </View>
      <View style={styles.timeStamp}>
        <Icon name="map" size={15} color="steelblue" />
        <Text style={styles.info}>{imageData?.location?.description}</Text>
      </View>
      <TouchableOpacity
        activeOpacity={0.5}
        style={styles.button}
